...
let userDataSchema = yup.object({
  ...
  phone: yup
    .string()
    // .matches(/^\+?[78][-(]?\d{3}\)?-?\d{3}-?\d{2}-?\d{2}$/, 'Неверный номер телефона.')
...
const onSubmit = handleSubmit.withControlled(async (values) => {
  ...
  try {
    ...
    const success = await handleSignup(userCred);
    if (success) {
      router.push("/menu");
    } else {
      throw new Error("Ошибка регистрации. Повторите попытку позже");
    }
  } catch (error) {
    errorText.value =
      ...
      <form class="main-data" @submit="onSubmit">
        <div class="personal-data">
          ...
          <div class="personal-details">
            ...
            />
            <p class="error-text" v-if="errors.repeatPassword">
        ...
        <div class="buttons">
          <button class="order-button" type="submit">
            <p v-if="!isLoading">Зарегистрироваться</p>
            <v-progress-circular
              v-if="isLoading"
              indeterminate
              color="primary"
